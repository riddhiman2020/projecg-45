...
function setup() {
  ...
  Bwall=createSprite(700,670,2000,20)
  Bwall.visible=false

  Twall=createSprite(700,-6400,2000,20)
  Twall.visible=false

  //creates parking icons to show where we have to park
  icon1=createSprite(1000,-800,20,20);
  icon1.addImage("icon",parking)
  icon1.scale=0.26

  icon2=createSprite(380,-2470,20,20);
  icon2.addImage("icon",parking)
  icon2.scale=0.26

  icon3=createSprite(380,-3180,20,20);
  ...
  //creates small sprite which are near the icons 
  I1=createSprite(1150,-800,5,5);
  I1.visible=false
  I2=createSprite(200,-2470,5,5);
  I2.visible=false
  I3=createSprite(200,-3180,5,5);
  I3.visible=false
  I4=createSprite(1150,-3970,5,5);
  I4.visible=false

  //creates fence
  Fence=createSprite(690,-1160,10,10)
  Fence.addImage("fence",fence);
  Fence.scale=0.28
  //the main car(PC)
...
function draw() {
  ...
  if(car1.isTouching(I1)){
    score=10;
    Fence.y=-2830;
  }

  if(car1.isTouching(I2)){
    score=20;
    Fence.y=-3440;
  }

  if(car1.isTouching(I3)){
    score=30;
    Fence.y=-4330;
  }

  if(car1.isTouching(I4)){
    score=40;
   Fence.y=4000;
  }

  if( car1.collide(Twall)){
    gameState=WIN;
  }

  if(car1.collide(Fence)||car1.collide(car2)||car1.collide(car3)||car1.collide(car4)||car1.collide(car5)){
    gameState=END
  }
 ...
 if(gameState===END){
  ...
  textSize(25);
  text("Score: "+score,20,car1.y-350)
}
